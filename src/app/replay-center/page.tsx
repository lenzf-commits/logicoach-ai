...
import { getUserReplayAnalyses } from "@/lib/queries/replay-analyses";
...
export default async function ReplayCenterPage({ searchParams }: ReplayCenterPageProps) {
  ...
  const [interviews, analyses] = await Promise.all([getUserInterviews(), getUserReplayAnalyses()]);
  const analysisByInterview = new Map(analyses.map((item) => [item.interview_id, readReplayAnalysis(item.report)]));
  ...
  return (
    <PageShell eyebrow="Replay" title="Replay Center" description="Finde deine vergangenen Trainings, lies deine Antworten mit gezieltem Feedback und starte direkt die nächste Runde.">
